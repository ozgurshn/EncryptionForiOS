...
+ (NSData*)decryptString:(NSString*)encryptedString RSAPrivateKey:(SecKeyRef)privateKey padding:(SecPadding)padding
{
    @try
    {
       size_t decryptedLength = SecKeyGetBlockSize(privateKey);
        //size_t decryptedLength = 2048;
        uint8_t decrypted[decryptedLength];
       
        const char* cStringValue = [encryptedString UTF8String];
        ...
        if(status == noErr)
        {
             NSLog(@"FINAL decrypted text: %s", decrypted);
...
+ (SecKeyRef)getPublicKeyFromBase64String:(NSString*)s_key withTag:(NSString *)tag
{
    // This will be base64 encoded, decode it.
    NSData *d_key = [NSData dataWithBase64EncodedString:s_key];
    d_key = [self stripPublicKeyHeader:d_key];
